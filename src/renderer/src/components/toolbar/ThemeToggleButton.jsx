...
const ThemeToggleButton = () => {
  ...
  const buttonClasses =
    'flex items-center justify-center w-12 h-12 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors duration-200'
  const svgIconClasses = 'w-6 h-6 text-gray-600 dark:text-gray-300'

  return (
    <button onClick={toggleTheme} className={buttonClasses} title="Toggle Theme">
      {theme === 'light' ? (
        <svg
